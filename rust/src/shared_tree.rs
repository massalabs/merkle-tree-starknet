...
pub struct SharedTree<'a> {
    pub bonsai_storage: BonsaiStorage<BasicId, RocksDB<'a, BasicId>>,
...
impl<'a> SharedTree<'a> {
    ...
    pub fn new(bonsai_storage: BonsaiStorage<BasicId, RocksDB<'a, BasicId>>) -> Self {
    ...
    pub fn insert(&mut self, k: Vec<u8>, value: &str) -> Result<(), BonsaiStorageError> {
        let key = BitVec::from_vec(k);

        let val = &Felt252Wrapper::from_hex_be(
            value
        ).map_err(|e| BonsaiStorageError::Trie("Error Felt252".to_string()))?;
